...
# Full copy of the delivered GC_MAPSHEET.gpkg with SOURCE_RC refreshed from
# PA_EXCEL_PATH — built once via scripts/build_custom_gc_mapsheet.py, not
# regenerated by this Makefile. Replaces the old lots/WU/mapsheets.geojson
# admin-zones pipeline as the --admin-zones input for `merge`.
GC_MAPSHEET_CUSTOM    := $(DELIVERY_DIR)GC_MAPSHEET_custom.gpkg
...
help:
	...
	$(call check_file,GC_MAPSHEET_CUSTOM,$(GC_MAPSHEET_CUSTOM))
...
$(MASTER_GDB)/timestamps: $(SOURCES_DIR)RC1.gdb $(SOURCES_DIR)RC2.gdb $(GC_MAPSHEET_CUSTOM)
	@_T_START=$$(date +%s); \
	\
	...
	gcover $(_GCOVER_FLAGS) publish merge \
		...
		--admin-zones $(GC_MAPSHEET_CUSTOM) \
	...
	[ $$rc -ne 0 ] && exit $$rc; \
	\
	echo "--- [2/2] Copying GC_MAPSHEET from $(GC_MAPSHEET_CUSTOM) ---"; \
	_T3=$$(date +%s); \
	ogr2ogr -f "OpenFileGDB" -update -overwrite -dim XY $(MASTER_GDB) \
		$(GC_MAPSHEET_CUSTOM) \
		-dialect SQLite \
		-sql "SELECT geom, MSH_MAP_TITLE, MSH_MAP_NBR, MSH_TOPO_NR, SOURCE_RC, Version AS VERSION, BER, ERL, ber_link AS BER_LINK, erl_link AS ERL_LINK FROM mapsheet_gc" \
		-nln GC_MAPSHEET; \
	_T4=$$(date +%s); \

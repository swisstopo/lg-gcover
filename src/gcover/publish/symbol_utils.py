    ...
    for layer in symbol_obj["symbolLayers"]:
        ...
        if layer_type == "CIMSimpleMarker":
            ...
            if nested_symbol:
                for nested_layer in nested_symbol.get("symbolLayers", []):
                    if nested_layer.get("type") == "CIMSolidFill":
                        result['color'] = extract_color_from_cim(nested_layer.get("color"))
                        break
            break
...
def should_use_override(symbol_layers: SymbolLayersInfo, 
...
def compare_symbols(old_symbol: SymbolLayersInfo, 
                   new_symbol: SymbolLayersInfo) -> Dict[str, Any]:
    """
    ...
    if len(old_symbol.fills) != len(new_symbol.fills):
        changes.append(f"Fill count changed: {len(old_symbol.fills)} → {len(new_symbol.fills)}")
...
def print_symbol_summary(symbol_layers: SymbolLayersInfo, name: str = "Symbol"):
    ...
    for i, fill in enumerate(symbol_layers.fills):
        ...
        if fill_type == 'solid':
            ...
            if marker:
                print(f"  {i+1}. Character: {marker.font_family} #{marker.character_index}")

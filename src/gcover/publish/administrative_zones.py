# src/gcover/publish/administrative_zones.py
"""
...
def _write_processing_metadata(gpkg_path: Path, input_files: dict[str, Path]) -> None:
    ...
    One row per input file plus one row for this module.
    """
    ...
    module_fp = _file_fingerprint(module_path)
    rows.append(("module", module_path.name, str(module_path.resolve()), module_fp["sha256"], module_fp["mtime"], module_fp["git_hash"], generated_at))
...
def create_administrative_zones(
    ...
    This function processes the 4 standardized source files into a single GPKG
    ...
    Example:
        create_administrative_zones(
            output_path=Path("gcover/data/administrative_zones.gpkg"),

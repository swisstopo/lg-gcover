...
def build_lookup_table(
    ...
    for idx, class_obj in enumerate(classification.classes):
        ...
        # Build symbol_id - priority:
        # 1. identifier_field from config (e.g., GMU_CODE for bedrock)
        # 2. class_obj.identifier if available
        # 3. Fall back to index
        identifier_value = idx  # Default
        ...
        for field_values in class_obj.field_values:
            ...
            # Determine identifier value for this specific value combination
            if identifier_field_idx is not None:
                # Use the value from the specified identifier field
                id_val = field_values[identifier_field_idx]
                if id_val and id_val != '<Null>':
                    identifier_value = id_val
            elif hasattr(class_obj, 'identifier') and class_obj.identifier:
                try:
...
def apply_batch_from_config_vectorized(
    ...
    for layer in layers_to_process:
        ...
        analysis_fields = set()
        for class_tuple in classifications_to_apply:
            classification = class_tuple[0]
            field_mapping = class_tuple[3] if len(class_tuple) > 3 else None

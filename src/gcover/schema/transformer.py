...
def extract_all_datasets(data: Any, path: str = "") -> Dict[str, List[Dict[str, Any]]]:
    ...
    logger.info(f"Extraction summary: {len(results['tables'])} tables, "
...
def create_field_from_esri_data(field_data: Dict[str, Any]) -> Field:
    ...
    return Field(
        ...
        domain=field_data.get("domain", {}).get("domainName") if "domain" in field_data else None,
    )
...
def create_relationship_from_esri_data(rel_data: Dict[str, Any]) -> RelationshipClass:
    ...
    if "originClassNames" in rel_data:
        origin_tables = [item.get("name", "") for item in rel_data["originClassNames"] if isinstance(item, dict)]
    
    if "destinationClassNames" in rel_data:
        dest_tables = [item.get("name", "") for item in rel_data["destinationClassNames"] if isinstance(item, dict)]
...
def extract_domains_from_esri_data(input_data: Dict[str, Any], target_prefix: str = "GC_") -> tuple[Dict[str, CodedDomain], Dict[str, RangeDomain]]:
    ...
    for domain_data in input_data["domains"]:
        ...
        if target_prefix and not get_base_table_name(domain_name).startswith(target_prefix):
            continue
    ...
    def find_subtypes(obj: Any, path: str = ""):
        ...
        if isinstance(obj, dict):
            ...
            if "subtypes" in obj and isinstance(obj["subtypes"], list):
                parent_name = obj.get("name", path.split(".")[-1] if path else "Unknown")
...
def transform_esri_json(
    ...
    try:
        ...
        coded_domains, range_domains = extract_domains_from_esri_data(input_data, target_prefix)
        schema.coded_domains = coded_domains
        schema.range_domains = range_domains
        logger.info(f"Found {len(coded_domains)} coded domains, {len(range_domains)} range domains")
        ...
        for table_data in all_datasets['tables']:
            ...
            if should_import_table(table_name, target_prefix, excluded_tables):
                try:
                    ...
                    if EXCLUDED_FIELDS:
                        table.fields = [f for f in table.fields if f.name not in EXCLUDED_FIELDS]
        ...
        for fc_data in all_datasets['feature_classes']:
            ...
            if should_import_table(fc_name, target_prefix, excluded_tables):
                try:
                    ...
                    # Filter fields if needed
                    if EXCLUDED_FIELDS:
                        feature_class.fields = [f for f in feature_class.fields if f.name not in EXCLUDED_FIELDS]

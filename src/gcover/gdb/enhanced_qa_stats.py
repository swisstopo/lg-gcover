...
class EnhancedQAConverter:
    ...
    def _get_previous_run_data(
        ...
        result = self.conn.execute(
            query,
            [verification_type, rc_version, test_name, issue_type, before_date]
        ).fetchone()
    ...
    def display_enhanced_summary(
        ...
        for result in test_results:
            ...
            table.add_row(
                ...
                f"{result.trend_data.previous_count:,}" if result.trend_data.previous_count else "—",
    ...
    def get_weekly_summary(
        ...
        for week_offset in range(weeks_back):
            ...
            if not results.empty:
                ...
                rc_summary = results.groupby('rc_version').agg({
                    'total_count': 'sum',
                    'num_runs': 'sum',
                    'latest_run': 'max'
                }).reset_index()
...
def enhanced_stats_command(
    ...
    if target_week:
        try:
            ...
            console.print(f"[red]Invalid week format: {target_week}. Use YYYY-MM-DD[/red]")
            return
    ...
    console.print(f"[dim]Filters: QA Type={qa_type}, Week={target_week}, Date={target_date}[/dim]")

...
def export_tables(
    ...
    if not HAS_ARCPY:
            click.echo("❌ Requires ArcGIS Pro")
            raise click.Abort()

    # ✅ Import TARDIF
    from gcover.schema.utils import _arcgis_table_to_df, _transform_table_data, _export_table_to_json
...
def extract(source, output, name, format, filter_prefix, remove_prefix):
    ...
    # ✅ Import TARDIF
    from gcover.schema import SchemaDiff, extract_schema

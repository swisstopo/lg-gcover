...
# TODO: remove
def get_frozen_layers(batch_config):
    """Get all layers with classes_mode=frozen."""
    frozen = []
    for layer in batch_config.layers:
        for classification in layer.classifications:
            mapfile_config = getattr(classification, "mapfile_config", None)
            if mapfile_config and mapfile_config.classes_mode == "frozen":
                frozen.append(layer)
                break  # One classification is enough
    return frozen
...
def generate_with_staging(
    layer_config, classification, output_dir: Path, diff_tool: Optional[str] = None
) -> Path:
    """
    Generate classes to staging and optionally launch diff.

    Args:
        layer_config: Layer configuration
        classification: Classification to generate
    ...
    generator = MapServerGenerator(
        layer_type=layer_config.layer_type,  # Or determine from gpkg
        use_symbol_field=False,  # Or from config
    )

    # Generate to staging
    staging_file = generator.generate_layer(
        data=classification.data,
        layer_name=layer_config.gcover_layer,
        layer_type=layer_config.layer_type,
        classification=classification,
        output_dir=output_dir,
        symbol_prefix=getattr(
            classification, "symbol_prefix", layer_config.gcover_layer
        ),
        staging_mode=True,  # ← KEY: staging mode
    )

    # staging_file is a Path to .staging/*.new
    staging_file = Path(staging_file)

    # Determine original file path
    original_file = staging_file.parent.parent / staging_file.name.replace(".new", "")
    ...
    if diff_tool:
        ...
        if not success:
            logger.info("")
            logger.info("Tip: Merge changes in diff tool, then save to original file")

...
& gcover   --env production  gdb process-all  --yes  --continue-on-error --filter-type verification_topology   --since $LAST_WEEK --max-workers 1  2>&1 | Tee-Object -FilePath $LogFile -Append

Write-Host "--- Processing new QA tests ---" -ForegroundColor Green
& gcover --env production  qa process-all  --qa-type topology --since $LAST_WEEK --max-workers 1 --format flatgeobuf  $InputDir   2>&1 | Tee-Object -FilePath $LogFile -Append

Write-Host "--- Processing QA aggregate ---" -ForegroundColor Green
& gcover  --env production qa aggregate --auto-discover --yes --zone-type mapsheets   --output-format xlsx   --type  verification_topology   --base-dir $OutputDir   2>&1 | Tee-Object -FilePath $LogFile -Append
& gcover   --env production qa aggregate --auto-discover --yes --zone-type lots   --output-format xlsx   --type  verification_topology   --base-dir $OutputDir   2>&1 | Tee-Object -FilePath $LogFile -Append
& gcover  --env production qa aggregate --auto-discover --yes --zone-type work_units   --output-format xlsx   --type  verification_topology   --base-dir $OutputDir     2>&1 | Tee-Object -FilePath $LogFile -Append

Write-Host "--- Processing QA extract ---" -ForegroundColor Green
& gcover    --env production qa extract  --yes --type verification_topology --format filegdb  --output $OutputDir    2>&1 | Tee-Object -FilePath $LogFile -Append

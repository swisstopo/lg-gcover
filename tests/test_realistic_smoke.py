...
def test_gdb_scan_count_assets(runner, test_data_exists):
    ...
    total_line = [
        line for line in lines if "Total:" in line and "GDB assets found" in line
    ]
...
def test_gdb_scan_handles_missing_data_gracefully(runner):
    ...
    content = source.read_text(encoding="utf-8").replace(
        "tests/data/examples", "dummy/data/examples"
    )
    ...
    for cmd in commands_to_test:
        try:
            ...
            assert result.exit_code in [0, 1, 2], (
                f"Command {cmd} crashed with code {result.exit_code}: {result.output}"
            )

            # Should not contain Python tracebacks (sign of unhandled exceptions)
            assert "Traceback" not in result.output, (
                f"Command {cmd} had unhandled exception: {result.output}"
            )
